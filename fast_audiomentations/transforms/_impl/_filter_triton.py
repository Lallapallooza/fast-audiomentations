...
def sinc_kernel(
    ...
    if mode == "high":
        ...
        adjusted_val = tl.where(
            pos == center_idx, 1.0 - normalized_sinc, -normalized_sinc
        )

        tl.store(
            output_ptr + batch_idx * num_taps + pos, adjusted_val, mask=mask
        )
    elif mode == "low":
        tl.store(
            output_ptr + batch_idx * num_taps + pos, normalized_sinc, mask=mask
        )
    else:
...
def unfold_kernel(
    ...
    for i in range(kernel_size):
        ...
        val = tl.where(
            in_bounds,
            tl.load(
                input_ptr + batch_idx * length + input_pos + i, mask=in_bounds
            ),
            0,
        )

        out_idx = (
            batch_idx * n_frames * kernel_size + frame_idx * kernel_size + i
        )
...
def unfold_triton(input, kernel_size, stride):
    ...
    grid = lambda META: (
        input.shape[0],
        triton.cdiv(n_frames, META["BLOCK_SIZE"])
        + (n_frames % META["BLOCK_SIZE"] != 0),
    )
...
def complex_mul_conjugate(a: torch.Tensor, b: torch.Tensor):
    ...
    return torch.stack(
        complex_mul_conjugate_triton(a_real, b_real, a_imag, b_imag), dim=-1
    )
...
def complex_mul_conjugate_triton(a_real, b_real, a_imag, b_imag):
    assert (
        a_real.shape[-1] == b_real.shape[-1]
    )  # Ensure last dimensions match for multiplication
...
def fft_conv1d(
    ...
    weight = F.pad(
        weight, (0, block_size - weight.shape[-1]), mode="constant", value=0
    )
    if weight.dtype != torch.float16 and weight.shape[1].bit_count() != 1:
        weight_z = torch.view_as_real(
            torch.fft.rfft(weight.to(torch.float32), dim=-1)
        ).to(torch.float16)
    else:
    ...
    if frames.dtype == torch.float16 and frames.shape[1].bit_count() != 1:
        frames_z = torch.view_as_real(
            torch.fft.rfft(frames.to(torch.float32), dim=-1)
        ).to(torch.float16)
    else:
    ...
    if out_z.dtype == torch.float16 and out_z.shape[1].bit_count() != 1:
        out = torch.fft.irfft(
            torch.view_as_complex(out_z.to(torch.float32)), block_size, dim=-1
        ).to(torch.float16)
    else:
