...
def rms_kernel(
    ...
    for offset in range(0, audios_max_len, BLOCK_SIZE_RMS):
        ...
        audios_vals = tl.load(
            audios + batch_idx * audios_max_len + audios_block_ptr,
            mask=audios_mask,
        )
        audios_partial_sum_sq = tl.where(
            audios_mask, audios_vals * audios_vals, 0
        )
...
@triton.autotune(
    configs=[
        ...
        for (block_size_sum, num_warps) in itertools.product(
            [512, 1024], [2, 4, 8, 16]
        )
    ],
...
def sum_with_snr_kernel(
    ...
    # 10**(x/20) computed as exp(x * ln(10)/20); ln(10)/20 is precomputed.
    desired_rms_val = tl.load(desired_rms + batch_idx)
    relative_rms = clean_audio_rms / tl.exp(
        desired_rms_val * 0.11512925464970228
    )
    ...
    for offset in range(0, clean_audio_max_len, BLOCK_SIZE_SUM):
        ...
        clean_audio_vals = tl.load(
            clean_audio
            + batch_idx * clean_audio_max_len
            + clean_audio_block_ptr,
        ...
        offset_adjusted = offset_over_max - tl.minimum(
            offset_over_max,
            tl.maximum(
                0,
                (offset_over_max + BLOCK_SIZE_SUM) - noisy_audio_real_lens_val,
            ),
        )
        ...
        noisy_audio_val = tl.load(
            noisy_audio_ptr
            + batch_idx * noisy_audio_max_len
            + noisy_audio_block_ptr,
        ...
        tl.store(
            ...
            + clean_audio_block_ptr,
            clean_audio_vals
            + noisy_audio_val * (relative_rms / noisy_audio_rms),
            mask=clean_audio_mask,
        )


def sum_with_snr_triton(
    samples: torch.Tensor,
    samples_lens: torch.Tensor,
    samples_noise: torch.Tensor,
    samples_noise_lens: torch.Tensor,
    snrs: torch.Tensor,
) -> torch.Tensor:
    assert samples.is_contiguous() and samples_noise.is_contiguous(), (
        "Samples must be contiguous"
    )

    B, T = samples.shape
    output = torch.empty_like(
        samples, device=samples.device, dtype=samples.dtype
    )
    ...
    sum_with_snr_kernel[grid](
        ...
        samples_noise,
        samples_noise_lens,
        samples_noise.shape[1],
        output,
        BLOCK_SIZE_RMS=max(
            1024,
            triton.next_power_of_2(max(T, samples_noise.shape[1]) // 1024),
        ),
    )
