...
def apply_gain_kernel(
    ...
    for i in range(0, audio_len, BLOCK_SIZE):
        ...
        samples = tl.load(
            samples_ptr + audio_idx * audio_len + sample_idx, mask=mask
        )
        result = samples * gain
        tl.store(
            output_ptr + audio_idx * audio_len + sample_idx, result, mask=mask
        )


def apply_gain(
    samples: torch.Tensor,
    amplitude_ratios: torch.Tensor,
    inplace: bool = False,
) -> torch.Tensor:
    ...
    if inplace:
        apply_gain_kernel[grid](
            samples, amplitude_ratios, samples, n_audios, audio_len
        )
        return samples
    copy = torch.empty_like(samples, device="cuda", dtype=samples.dtype)
    apply_gain_kernel[grid](
        samples, amplitude_ratios, copy, n_audios, audio_len
    )
    return copy
